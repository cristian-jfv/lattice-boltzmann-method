...
int main()
{
  ...
  const double rho_inlet = rho_outlet; //3.0*(H-1)*p_grad + rho_outlet;
  ...
  // Tensors
  const Tensor Fg = torch::tensor({{-0.0003},{0.0}});
  print("Fg", Fg);
  ...
  for (int t=0; t < T; t++)
  {
    ...
    // Add the force density to the velocity
    u += Fg.t();

    // Compute equilibrium
    //print("compute equilibrium");
    solver::incomp_equilibrium(f_equi, u, rho);
    equi_populations.copy_(-omega*( f_adve - f_equi ));

    // Force source terms
    Tensor S = ((1-0.5*omega)*((ics2 + ics4*u.matmul(c))*Fg.t().matmul(c) - ics2*u.matmul(Fg))*E).clone().detach();

    // Collision, BGK operator
    //print("collision step");
    f_coll.copy_(
      f_adve + equi_populations + S
    );
  ...
  torch::save(ux, "gt-ux.pt");
  torch::save(uy, "gt-uy.pt");
  torch::save(fs, "gt-fs.pt");
  torch::save(rhos/3.0, "gt-ps.pt");
